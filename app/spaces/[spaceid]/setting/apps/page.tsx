...
export default function ExploreAppsPage() {
  ...
  > = {
    ...
    CommunityApps: {
      ...
      getApps: async () => {
        try {
          const dapps = await ExternalDAppService.getAllDAppsInUnifiedFormat();
          return dapps.filter((dapp) => dapp.isInstallable);
        } catch (error) {
          console.error('Error fetching external dApps:', error);
          return [];
        }
      },
    },
...
function AppList(props: { appsPromise: Promise<(Dapp | NativeDApp)[]> }) {
  ...
  return (
    <>
      {apps.map((app) => (
        <AppItem key={'id' in app ? app.id : app.appIdentifier} data={app} />
      ))}
    </>
  );
}
