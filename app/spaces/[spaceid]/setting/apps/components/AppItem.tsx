...
const AppItem = (props: Props) => {
  ...
  const { appName, categories: _categories = '', appLogoUrl } = data;
  ...
  const handleInstall = useCallback(() => {
    ...
    let params: InstallDAppParams;

    if (isNativeDApp(data)) {
      // Native app
      params = {
        spaceId,
        appId: '',
        nativeAppName: data.appIdentifier,
      };
    } else {
      // External or local dApp
      const isExternal = data.id.startsWith('external_');
      params = {
        spaceId,
        appId: data.id,
        nativeAppName: '',
        ...(isExternal && { externalAppData: data }),
      };
    }
      ...
      >
        ...
        >
          ...
          >
            ...
            <Image
              alt={data.developerName || 'Developer'}
              src={data.appLogoUrl ?? USER_AVATAR_URL}
