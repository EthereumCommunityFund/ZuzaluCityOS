...
import { ExternalDAppService } from '@/services/external-dapps';
...
export default function List({ onDetailClick, onOwnedDappsClick }: ListProps) {
  ...
  // Old data sources (commented out for now)
  ...
  // New external data source from project_snaps
  const {
    data: externalDappData,
    isLoading: externalDappLoading,
    error: externalDappError,
  } = useQuery({
    queryKey: ['GET_EXTERNAL_DAPP_LIST_QUERY'],
    queryFn: () => ExternalDAppService.getAllDAppsInUnifiedFormat(),
  });

  // Set data and loading states
  const data = externalDappData || [];
  const isLoading = externalDappLoading;
  const legacyDappData: any[] = [];
  const legacyDappLoading = false;
  ...
  const ownedDapps: any[] = [];

  const isAllLoading = isLoading || legacyDappLoading;

  return (
    <div className="flex flex-1 flex-col gap-[20px] p-[20px] pr-[35px] mobile:gap-[10px] mobile:p-[20px_10px]">
          ...
          ? Array.from({ length: 4 }).map((_, index) => (
              <ResponsiveGridItem key={index}>
                <div className="p-[10px]">
                  <Skeleton className="mb-[20px] aspect-square h-auto w-full rounded-[10px]" />
